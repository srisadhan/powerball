...
Vector<6, float> Cd_diag = makeVector(1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f) * 160; // for High damping
...
int main(int argc, char **argv)
{
    ...
    {
        std::cout << "Improper task type selected, Please select from: [hf, hg, hc, lf, lg, lc]" << std::endl;
        return 0;
    }

    folder1 = "../data/IEEE_haptics/" + SubName + "/";
    folder2 = "../data/IEEE_haptics/" + SubName + "/" + task + "/";
    ...
    // set the damping based on the task config (First letter: admittance)
    if (std::tolower(taskType.c_str()[0]) == 'l')
        Cd_diag = makeVector(1.0f, 1.0f, 1.0f, 1.0f, 1.0f, 1.0f) * 160; // for High damping - Low admittance
    else if (std::tolower(taskType.c_str()[0]) == 'h')
        Cd_diag = makeVector(20.0f, 20.0f, 20.0f, 6.0f, 6.0f, 6.0f) * 2; // for Low damping - High admittance
    else if (std::tolower(taskType.c_str()[0]) == 'c'){
        Cd_diag = makeVector(20.0f, 20.0f, 20.0f, 20.0f, 20.0f, 20.0f) * 4; // for combined task and combined damping
        damp_adaptation = true;}
    else
        return 0;
    ...
    dataFile << "TimeStamp , Q1, Q2, Q3, Q4, Q5, Q6, dQ1, dQ2, dQ3, dQ4, dQ5, dQ6, FT1, FT2, FT3, FT4, FT5, FT6, X, Y, Z" << std::endl;
    // timestamp vars
    char buffer[10];
    struct timeval tv;
    time_t curtime;
    ...
    while ((cnt < 180 / dt) && (!stop_flag))
    {
        ...
        dataFile << Q[0] << "," << Q[1] << "," << Q[2] << "," << Q[3] << "," << Q[4] << "," << Q[5] << ","
                 ...
                 << FT[0] << "," << FT[1] << "," << FT[2] << "," << FT[3] << "," << FT[4] << "," << FT[5] << "," << X[0] << "," << X[1] << "," << X[2]<< std::endl;
